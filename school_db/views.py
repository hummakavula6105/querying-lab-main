...
def problem_one(request):
    ...
    students = Student.objects.all().filter(gpa__gt=3.0).order_by('-gpa')

    for each in range(len(students)):

      first_name = students[each].first_name

      last_name = students[each].last_name

      gpa = students[each].gpa

      print(f'Full Name: {first_name} {last_name} GPA: {gpa}')
...
def problem_two(request):
  ...
  instructors = Instructor.objects.all().filter(hire_date__lt='2010-01-01').order_by('hire_date')

  for each in range(len(instructors)):

      first_name = instructors[each].first_name

      last_name = instructors[each].last_name

      hire_date = instructors[each].hire_date
...
def problem_three(request):
  ...
  instructors = Instructor.objects.all().filter(id=2)
  courses = Course.objects.all().filter(instructor_id=2)


  first_name = instructors[0].first_name
  ...
  for each in range(len(courses)):
    ...
    course += f'-{courses[each].name} \n'

  print(f'Instructor Name: {first_name} {last_name}')
...
def problem_four(request):
  ...
  print(f'Student Count: {num_students}')
  print(f'Courses Count: {num_courses}')
  print(f'Instructor Count: {num_instructors}')
...
def problem_five(request):
    ...
    # year =2022
    # gpa = 3.0
    # new_student = Student.objects.create(first_name=first_name , last_name=last_name, id=id, year=year, gpa=gpa)
  ...
  new_student = Student.objects.update_or_create(first_name='Kyle', last_name='Harwood', id=11, year=2022, gpa=3.0)
...
def problem_six(request):
    ...
    Student.objects.filter(id=11).update(first_name = 'Kyle', last_name = 'Harwood' , gpa = 3.5)
